...
- **Text-blob parity:** Every text-blob type (beat, canon, part_notes, chapter_notes, scratch, guide) must support: get_context read (with section-level lazy loading), write (overwrite + append), and edit (surgical find-replace). When adding a new blob type, implement all three.
